...
class Api extends MY_Controller
{
	...
	function testnotification()
	{
		...
		$massage = 'Ahoy! Some of your friend wants to date you & has sent you an anonymous message on MTJF. To read and reply, sign-up on the MTJF ';
		$msgss = array('title'=>'New Match for Getting Naughty','msg'=>$massage,'image'=>'','type'=>5,'match_type'=>0,'create_at'=>militime);
	...
	// 	$sms_msg = "Ahoy! Some of your friend wants to date you & has sent you an anonymous message on MTJF. To read and reply, sign-up on the MTJF app."."\n"."http://onelink.to/rbzmmn";	
	// 	 $sms_msg = "Ahoy! Some of your friend wants to date you and have secretly liked you on MTJF. To find out who it is, sign-up on the MTJF app now."."\n"."http://onelink.to/rbzmmn";	
	// 	//echo $sms_msg;exit;
	// 	$aa =   $this->common_model->twilio_sms('+15857296736',$sms_msg);
	// 	print_r($aa);exit; 
	...
	function login()
	{
    	...
    	if($json_array->user_contact!='')
    	{
    		...
    		$seleuser = $this->common_model->common_getRow('mtjf_user',array('user_country_id'=>$json_array->user_country_id,'user_contact'=>$contact));
    		if(!empty($seleuser))
    		{
	    	...
	    	{	
	    		...
	    		if($updateotp!=false)
	    		{
					if($checkotp->first_login ==0)
					{
						...
						$updatedevicetoken = $this->common_model->updateData('mtjf_notification',array('receiver_id'=>$json_array->user_id),array('receiver_number'=>$checkotp->user_country_id.$checkotp->user_contact,'receiver_id'=>0)); //prequal notification view
					}
 ...
 //   	  			//$final_output['data'] = array('friend_status'=>$status,'wallet'=>$new_wallet);
	...
	{	
		...
		if($aa['status']=='true')
		{
			...
			{
				$like_id = $seluservote->like_id;
				if($seluservote->like_status != $like_status)
				{
					$response = 'true';
					$stap = 1;
				}else
						...
						if($likes!=false)
						{
							...
							}else
							{
								...
								if($like_hint!='')
								{
									...
									$sms_msg_twilio = "Ahoy! Some of your friend wants to date you & has sent you an anonymous message on MTJF. To read and reply, sign-up on the MTJF app."."\n"."http://onelink.to/rbzmmn";	
									...
									$sms_msg_twilio = "Ahoy! Some of your friend wants to date you and have secretly liked you on MTJF. To find out who it is, sign-up on the MTJF app now."."\n"."http://onelink.to/rbzmmn";		
									$notify_msg = "You have a new Fan!"; $type=3;
								}	
								$mtype = 0;
							}
							...
							if($frndid==0)
							{
								...
								if(!empty($checksms))
								{
									...
									if($hourdiff > 24)
									{
										...
										{
											$this->common_model->twilio_sms($contact_no,$sms_msg_twilio);
										}
									}else
									{
							...
							if(!empty($devicetoken))
							{
								...
								{
									$message = array('title'=>$title,'msg'=>$notify_msg,'contact_no'=>$aa['data']->user_country_id.$aa['data']->user_contact,'type'=>$type,'match_type'=>$mtype,'create_at'=>militime);
									$this->common_model->ios_notification($devicetoken,$message);
								}	
							}
	...
	function reply_on_hint()
	{
		...
		if($aa['status']=='true')
		{
			...
			if(!empty($json))
            {
				...
				if($data->like_id != '' && $data->message != '')
				{	
					$seleclike = $this->common_model->common_getRow("mtjf_user_like_unlike",array('like_id'=>$data->like_id,'like_status'=>1));
					if(!empty($seleclike))
					{
						$cont_name ='FAN';
						$userids = $this->db->query("SELECT user_id,user_name FROM mtjf_user WHERE 
									CONCAT(user_country_id,user_contact) = '".$seleclike->contact_no."'")->row();
						if(!empty($userids))
						{	
							$useridss = $userids->user_id;
						}else
						{
							$useridss = 0;
						}
						if($user_id != $seleclike->user_id)
						{
							 $contact_name1 = $this->db->select('contact_name')->get_where("mtjf_user_contact_list",array("user_id"=>$seleclike->user_id,'contact_no'=>$seleclike->contact_no))->row();
							if(!empty($contact_name1))
							{
								$cont_name = $contact_name1->contact_name;
							}
						}

						$response = 'true';
						$matchuser = $this->db->query("SELECT like_status FROM mtjf_user_like_unlike WHERE user_id = '".$useridss."' AND second_user_id = '".$seleclike->user_id."' ")->row();
						if(!empty($matchuser))
						{	
							if($matchuser->like_status == 1)
							{
								$response = 'false';
							}
						}
						if($response=='true')
						{
							...
							if(!empty($insert))
							{
								...
								//
							  	$msg = array('title'=>$cont_name,'msg'=>$data->message,'message_id'=>$insert,'old_message_id'=>$data->message_id,'image'=>'','type'=>1,'match_type'=>0,'like_id'=>$data->like_id,'user_id'=>$data->second_user_id,'contact_no'=>$aa['data']->user_country_id.$aa['data']->user_contact,'contact_name'=>$cont_name,'create_at'=>militime);
						...
						}else
						{
							//match
							$final_output["status"] = "failed";
                    		$final_output["message"] = "You cannot reply to this conversation.";
							$final_output['msg'] = "Replies to anonymous chat can not be sent if you get a Match with your Fan or if your Fan undoes liking you.";
						}
					}else
					{
						$final_output["status"] = "failed";
			...
			if(!empty($json))
            {
            	...
            	{ 
					...
					if(!empty($seleclike))
					{
						...
						$response = 'true';
						$matchuser = $this->db->query("SELECT like_status FROM mtjf_user_like_unlike WHERE user_id = '".$userids."' AND second_user_id = '".$seleclike->user_id."' AND like_status = 1 ")->row();
						if(empty($matchuser))
						{	
		            		if($data->create_at == 0)
		            		{ 
	...
	function interest()
	{
		...
		{
			$user_id = $aa['data']->user_id;
			$image = $aa['data']->user_image;
			$json = file_get_contents('php://input');
			if(!empty($json))
            {
            	$data = json_decode($json);
            	$final_output = array();
            	$insertt = 'failed'; $response = 'failed'; $new_wallet = "0";
            	if($data->interest_id != '' && $data->full_mobile_number != '' && $data->full_mobile_number != 0)
            	{
            		...
            		if(!empty($userdata))
            		{
		            	...
		            	$firstmatch = $this->db->query("SELECT like_id FROM mtjf_user_like_unlike WHERE user_id = '$user_id' AND contact_no = '".$data->full_mobile_number."' AND like_status = 1")->row();
		            	$secondtmatch = $this->db->query("SELECT like_id FROM mtjf_user_like_unlike WHERE user_id = '".$userdata->user_id."' AND contact_no = '".$aa['data']->user_country_id.$aa['data']->user_contact."' AND like_status = 1")->row();
		            	if(!empty($firstmatch) && !empty($secondtmatch))
		            	{
		            		...
		            		if($insertt=='true')
		            		{
		            			if($type==1)
		            			{
		            				...
		            				if($res==3)
			            			{
						        	    ...
						        	    $newinsertt = $this->common_model->updateData("mtjf_interest_data_store",array('response'=>$res,'status'=>$res,'interest_'.$data->interest_id =>$res,'update_date'=>date('Y-m-d H:i:s')),array('user_id'=>$userdata->user_id,'contact_user_id'=>$user_id));
			            				$mtype= 1;
		            					$msgss = array('title'=>'New Match for '.$interestmsg[$data->interest_id],'msg'=>$massage,'image'=>$image1,'type'=>$type,'match_type'=>1,'create_at'=>militime);
			            			}else
			            			{
			            				...
			            				$mtype= 0;
		            					$msgss = array('title'=>'You have new Interest' ,'msg'=>$massage_new,'image'=>$image1,'type'=>$type,'match_type'=>0,'create_at'=>militime);
			            			}
		            				if($userdata->user_device_type=='android')
	            					{
		            	...
		            	{
		            		$final_output["status"] = "failed";
		                    $final_output["message"] = "You are no longer matched with ".$userdata->user_name.".";
		            	}
            		}else
            		{
            			$final_output["status"] = "failed";
	...
	function Get_interest_detail()
	{
		...
		if($aa['status']=='true')
		{
			...
			$userdata = $this->db->query("SELECT user_id,user_name FROM mtjf_user WHERE CONCAT(user_country_id,user_contact) = '$user_cont_num' AND user_status = 1")->row();
			if(!empty($userdata))
			{
				$firstmatch = $this->db->query("SELECT like_id FROM mtjf_user_like_unlike WHERE user_id = '$user_id' AND contact_no = '".$user_cont_num."' AND like_status = 1")->row();
            	$secondtmatch = $this->db->query("SELECT like_id FROM mtjf_user_like_unlike WHERE user_id = '".$userdata->user_id."' AND contact_no = '".$aa['data']->user_country_id.$aa['data']->user_contact."' AND like_status = 1")->row();
            	if(!empty($firstmatch) && !empty($secondtmatch))
            	{
		...
		if($aa['status']=='true')
		{
			...
			if(!empty($notificationdata))
			{
				foreach ($notificationdata as $key) {
					...
					$image = $u_name = ''; $sender_num = ''; $like_id = 0;
						if($key->type!=3)
						{
							$userimage= $this->db->select('user_image,user_country_id,user_contact,user_name')->get_where("mtjf_user",array('user_id'=>$key->sender_id))->row();
							if(isset($userimage))
							{ 
								if($key->type!=9)
								{
									if(isset($userimage->user_image) && $userimage->user_image!='')
									{
								...
								$sender_num = $userimage->user_country_id.$userimage->user_contact;
								$u_name = $userimage->user_name;
							}
						}

					$likeid = $this->db->query("SELECT like_id FROM mtjf_user_like_unlike WHERE (like_status = 1 AND user_id = '".$key->sender_id."') AND (second_user_id = '".$user_id."' OR contact_no = '".$aa['data']->user_country_id.$aa['data']->user_contact."') ")->row();
					if(!empty($likeid))
					{
						$like_id = $likeid->like_id;
					}	
                ...
                if($data->full_mobile_number != '')
                {
                	...
                	if(!empty($checkexisting))
	            	{
	            		...
	            		if($update !=false)
	            		{
	            			...
	            			$contactname = $this->db->select('contact_name')->get_where("mtjf_user_contact_list",array('user_id'=>$user_id,'contact_no'=>$$data->full_mobile_number))->row();
	            			if(!empty($contactname))
	            			{
	            				$cont_name = $contactname->contact_name;
	            			}else
	            			{
	            				$cont_name = '';
	            			}
	            			$userid = $this->db->query("SELECT user_id,user_device_type,user_device_token FROM mtjf_user WHERE CONCAT(user_country_id,user_contact) = '".$data->full_mobile_number."' ")->row();
        					if(!empty($userid->user_id))
        					{
        						...
        						$msg = array('title'=>'','msg'=>'','contact_no'=>$aa['data']->user_country_id.$aa['data']->user_contact,'image'=>'','type'=>11,'match_type'=>0,'user_name'=>$cont_name,'create_at'=>militime);
								if(!empty($userid->user_device_token) && $userid->user_device_type == 'android')
								{
								   $this->common_model->sendPushNotification($userid->user_device_token,$msg);	
								}elseif(!empty($userid->user_device_token))
								{
								 	$this->common_model->ios_notification($userid->user_device_token,$msg);
								}
        					}
    						$final_output["status"] = "success";
		...
		{ 
            ...
            {   
                ...
                if($dcount!=0)
                {
	                ...
	                $newdata = array(); $contactarrimp = 0; $insertdata = false;
                	if(!empty($contactarr))
                	{
	...
	function AppVersion()
	{
		...
		if($type=='android')
		{
	    	$version = $this->db->query("SELECT android_version FROM app_version WHERE min_android_version <= '$app_version'")->row();
		}else
		{
	    	$version = $this->db->query("SELECT ios_version FROM `app_version` WHERE `min_ios_version` <= '$app_version'")->row();
		}
		if(!empty($version))
	    {
            $final_output['status']='success';
	    ...
	    {
	        $final_output['status']='failed';
            $final_output['message']='We no longer support this version, Please update the application.';
            unset($final_output['data']);       
	    }   
		header("content-type: application/json");
	    echo json_encode($final_output);	
	...
	{
		$auth = $this->common_model->getDataField('user_id,user_facebook_id,user_country_id,user_contact,user_name,user_gender,user_image','mtjf_user',array('user_token'=>$token));
		if(!empty($auth))
		{
			$abc['status'] = "true";
			$abc['data'] = $auth[0];
